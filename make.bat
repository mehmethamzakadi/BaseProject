@echo off
REM ============================================
REM BaseProject - Windows Batch Script
REM ============================================
...
set "API_CONTAINER_DEV=baseproject_api_dev"
set "API_CONTAINER_PROD=baseproject_api_prod"
set "DB_CONTAINER_DEV=baseproject_postgres_dev"
...
echo.
echo ============================================
echo   BaseProject - Windows Komutlari
echo ============================================
echo.
...
docker ps | findstr baseproject >nul 2>nul
if %ERRORLEVEL% EQU 0 (
...
docker ps --filter "name=baseproject" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
goto end
...
:logs-api
%COMPOSE_DEV% logs -f baseproject.api
goto end

:logs-client
%COMPOSE_DEV% logs -f baseproject.client
goto end
...
docker exec -it %API_CONTAINER_DEV% dotnet ef migrations add %NAME% --project /app/BaseProject.Persistence.csproj --output-dir Migrations/PostgreSql --context BaseProjectDbContext
if %ERRORLEVEL% EQU 0 (
...
docker exec -it %API_CONTAINER_DEV% dotnet ef database update --project /app/BaseProject.Persistence.csproj --context BaseProjectDbContext
if %ERRORLEVEL% EQU 0 (
...
docker exec -it %API_CONTAINER_DEV% dotnet ef migrations remove --project /app/BaseProject.Persistence.csproj --context BaseProjectDbContext
if %ERRORLEVEL% EQU 0 (
...
docker exec -it %API_CONTAINER_DEV% dotnet ef migrations list --project /app/BaseProject.Persistence.csproj --context BaseProjectDbContext
if %ERRORLEVEL% NEQ 0 (
...
docker exec -it %DB_CONTAINER_DEV% psql -U postgres -d BaseProjectDb
if %ERRORLEVEL% NEQ 0 (
...
docker exec -it baseproject_client_dev /bin/sh
if %ERRORLEVEL% NEQ 0 (
...
docker exec -it baseproject_ollama_dev ollama pull %MODEL%
if %ERRORLEVEL% NEQ 0 (
...
docker exec -it baseproject_ollama_dev ollama list
if %ERRORLEVEL% NEQ 0 (
...
docker exec -it %API_CONTAINER_DEV% dotnet run --project /app/BaseProject.API.csproj
if %ERRORLEVEL% NEQ 0 (
