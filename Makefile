...
# Container isimleri
API_CONTAINER_DEV  := baseproject_api_dev
API_CONTAINER_PROD := baseproject_api_prod
DB_CONTAINER_DEV   := baseproject_postgres_dev
DB_CONTAINER_PROD  := baseproject_postgres_prod

# ============================================
# Yardım Menüsü
# ============================================
help:
	@echo "$(GREEN)BaseProject - Makefile Komutları$(NC)"
	@echo ""
...
stop:
	...
	@if docker ps | grep -q baseproject; then \
...
health:
	...
	@docker ps --filter "name=baseproject" --format "{{.Names}}" | while read container; do \
...
migrate:
	...
	@docker exec -it $(API_CONTAINER_DEV) dotnet ef migrations add $(NAME) \
		--project /app/BaseProject.Persistence.csproj \
		--output-dir Migrations/PostgreSql \
		--context BaseProjectDbContext || \
